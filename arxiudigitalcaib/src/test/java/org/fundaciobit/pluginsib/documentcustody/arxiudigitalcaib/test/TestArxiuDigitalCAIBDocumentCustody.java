package org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test;
...
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.ArxiuDigitalCAIBDocumentCustodyPlugin;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.ExpedientCarpetaDocument;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Anexo;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Interesado;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Libro;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Oficina;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Organismo;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.RegistroDetalle;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.RegistroEntrada;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.TipoDocumental;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.Usuario;
import org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib.test.beans.UsuarioEntidad;
...
public class TestArxiuDigitalCAIBDocumentCustody extends TestDocumentCustody {
    ...
    public static void main(String[] args) {
        ...
        TestArxiuDigitalCAIBDocumentCustody test = new TestArxiuDigitalCAIBDocumentCustody();
    ...
    protected void compareDocument(String titol, AnnexCustody docSet, AnnexCustody docGet, boolean compareData) {
        ...
        if (docSet == null || docGet == null) {
            Assert.fail(titol + ".- Algun dels dos valors val null i l'altre no: [SET = " + docSet + "][GET = " + docGet
                    + "]");
        }

        Assert.assertEquals(titol + " Els noms no són iguals", docSet.getName(), docGet.getName());

        Assert.assertEquals(titol + " Les longituds no són iguals", docSet.getLength(), docGet.getLength());

        Assert.assertEquals(titol + " Els mimes no són iguals", docSet.getMime(), docGet.getMime());
        ...
        if (compareData) {
            ...
            Assert.assertNull(titol + ".- Les dades del fitxer haurien de ser NUll i valen alguna cosa",
                    docGet.getData());
        }
    }
    ...
    public void testMetadades() throws Exception {
        ...
        org.fundaciobit.pluginsib.documentcustody.api.DocumentCustody document = null;
        /*
        ...
        InputStream is2 = FileUtils.readResource(this.getClass(), "testarxiudigitalcaib/Firma2.pdf");
        ...
        Assert.assertNull("M2 La metadada amb clau " + key + " ha sigut esborrada però encara la puc llegir", mGet);
    ...
    public void testSimpleDocConfigurable(boolean reservarCadaVegada, boolean deleteOnFinish) throws Exception {
        ...
        Metadata[] metas = null;
        {
            ...
            InputStream is = FileUtils.readResource(this.getClass(), "testarxiudigitalcaib/Firma.pdf");
            ...
            if (reservarCadaVegada) {
                ...
                signatures = new SignatureCustody[] { null, signature, signature, signature, signature2 };
        ...
        for (int i = 0; i < signatures.length; i++) {
            ...
            // ----------- SIGNATURE
            {
                ...
                if (sign == null) {
                    Assert.assertNull("S1.- S'ha guardat una signatura NULL però retorna valors", binData);
                } else {
    ...
    protected Anexo getAnexo(RegistroEntrada registro) {
        ...
        Anexo anexo = new Anexo(222L, "Titulo Anexo", tipoDocumental, validezDocumento, tipoDocumento,
                registro.getRegistroDetalle(), "Observacions de l'Anexo", origenCiudadanoAdmin, fechaCaptura, modoFirma,
                "AdES-EPES");
        return anexo;
    }
    ...
    protected RegistroEntrada getRegistro() throws Exception {
        ...
        RegistroDetalle registroDetalle = new RegistroDetalle("Prova de Arxiu Digital CAIB ", tipoDocumentacionFisica,
                idioma, codigoAsunto, oficinaOrigen, interesados);

        Libro llibre = new Libro(62L, "LLIB");

        RegistroEntrada registro = new RegistroEntrada(12345L, usuarioEntitat, oficina, destino, destino.getCodigo(),
                destino.getDenominacion(), fecha, numeroRegistro, numeroRegistroFormateado, registroDetalle, llibre);
        return registro;
    }
    ...
    public IDocumentCustodyPlugin instantiateDocumentCustodyPlugin(Properties specificProperties)
        ...
        try {
            ...
            log.error("Error llegint fitxer plugin.properties: " + e.getMessage(), e);
            return null;
        }
        ...
        documentCustodyPlugin = (IDocumentCustodyPlugin) PluginsManager
                .instancePluginByClass(ArxiuDigitalCAIBDocumentCustodyPlugin.class, getPackageBase(), fsProperties);
    ...
    public String getPropertyBase() {
        return getPackageBase() + ArxiuDigitalCAIBDocumentCustodyPlugin.ARXIUDIGITALCAIB_PROPERTY_BASE;
    }
    ...
    public void testInternalMetadata() throws Exception {
        ...
        */
        ...
        System.out.println("Resultat Actualitzar DOCUMENT: " + rs.getCodigoResultado() + ": " + rs.getMsjResultado());
    ...
    @Test
    public void test3Combinacions() throws Exception {
    ...
    protected void checkEmptySignature(ArxiuDigitalCAIBDocumentCustodyPlugin plugin, String custodyID)
            throws CustodyException {
        byte[] dataSign = plugin.getSignature(custodyID);
        SignatureCustody sign1 = plugin.getSignatureInfo(custodyID);
        SignatureCustody sign2 = plugin.getSignatureInfoOnly(custodyID);
    ...
    protected void checkSign(SignatureCustody original, byte[] dataFile, SignatureCustody file1,
            SignatureCustody file2) {
