package org.fundaciobit.pluginsib.documentcustody.arxiudigitalcaib;
...
import org.fundaciobit.pluginsib.documentcustody.api.AbstractDocumentCustodyPlugin;
...
public class ArxiuDigitalCAIBDocumentCustodyPlugin extends AbstractPluginProperties implements IDocumentCustodyPlugin {
    ...
    public static final String ARXIUDIGITALCAIB_PROPERTY_BASE = DOCUMENTCUSTODY_BASE_PROPERTY + "arxiudigitalcaib.";
    ...
    static {
        ...
        TIPOFIRMA_COMPLEXE.put(SignatureCustody.XADES_SIGNATURE + "_false", TiposFirma.XADES_INTERNALLY);
    ...
    public String getPropertySolicitantIdentificadorAdministratiuEL() throws Exception {
        return getProperty(ARXIUDIGITALCAIB_PROPERTY_BASE + "connexio.solicitant.identificador_administratiu_EL");
    }
    ...
    public String getPropertyUsuariIdentificadorAdministratiuEL() throws Exception {
        return getProperty(ARXIUDIGITALCAIB_PROPERTY_BASE + "connexio.usuari.identificador_administratiu_EL");
    }
    ...
    @Override
    public String reserveCustodyID(Map<String, Object> custodyParameters) throws CustodyException {
        ...
        try {
            ...
            final String nomCarpeta = processELNullIfEmpty(getPropertyNomCarpetaEL(), custodyParameters);
            ...
            if (isSearchIfExpedientExistsInReserve()) {
                ...
                if (hiHaErrorEnCerca(res.getCodigoResultado())) {
                    throw new CustodyException("Error Consultant si Expedient " + nomExpedient + " existeix: "
                ...
                if (nomCarpeta != null) {
                    ...
                    if (nodos == null || nodos.size() == 0) {
                        ...
                        for (Nodo nodo : nodos) {
                            ...
                            if (nomCarpeta.equals(nodo.getName()) && TiposObjetoSGD.DIRECTORIO.equals(nodo.getType())) {
            ...
            if (hiHaError(resCSV.getCodigoResultado())) {
                throw new CustodyException(
                        "Error cridant a generar CSV: " + resCSV.getCodigoResultado() + "-" + resCSV.getMsjResultado());
            }
    ...
    @Override
    public String getCsv(String custodyID, Map<String, Object> custodyParameters) throws CustodyException {
        ...
        try {
            ...
            throw new CustodyException(
                    "Error intentant obtenir el CSV de la custòdia amb id " + custodyID + ": " + e.getMessage(), e);
        }

    }

    @Override
    public String getCsvValidationWeb(String custodyID, Map<String, Object> parameters) throws CustodyException {
    ...
    @Override
    public String getCsvGenerationDefinition(String custodyID, Map<String, Object> parameters) throws CustodyException {

        String csvGenerationDefinitionEL = getProperty(getPropertyBase() + "csv_generation_definition");
     ...
     */
    @Override
    public String getOriginalFileUrl(String custodyID, Map<String, Object> parameters) throws CustodyException {
    ...
    protected String getValidationUrlCommon(String custodyID, Map<String, Object> parameters, final String property,
            final String prefix) throws CustodyException {
        final String baseUrl = null;
        String baseUrlEL = getProperty(getPropertyBase() + property);

        String hashPassword = getProperty(getPropertyBase() + AbstractDocumentCustodyPlugin.ABSTRACT_HASH_PASSWORD, "");

        // Valid values MD2, MD5, SHA,SHA-256,SHA-384,SHA-512
        String hashAlgorithm = getProperty(getPropertyBase() + AbstractDocumentCustodyPlugin.ABSTRACT_HASH_ALGORITHM,
                "MD5");
        ...
        return AbstractDocumentCustodyPlugin.getValidationUrlStatic(custodyID, getCsv(custodyID, parameters),
                parameters, baseUrl, baseUrlEL, hashAlgorithm, hashPassword, prefix, log);
    }

    @Override
    public String getPrintableFileUrl(String custodyID, Map<String, Object> parameters) throws CustodyException {
    ...
    @Override
    public String getEniFileUrl(String custodyID, Map<String, Object> parameters) throws CustodyException {
    ...
    @Override
    public String getValidationFileUrl(String custodyID, Map<String, Object> parameters) throws CustodyException {
    ...
    public String getSpecialValue(String custodyID, Map<String, Object> custodyParameters) throws CustodyException {
        ...
        if (specialValue == null || specialValue.trim().length() == 0) {
            ...
            return AbstractDocumentCustodyPlugin.processExpressionLanguage(specialValue, custodyParametersExtended);
        }
    }
    ...
    public void deleteCustody(String custodyID) throws CustodyException, NotSupportedCustodyException {
        ...
        try {
            ...
            throw new CustodyException("Error esborrant expedient amb uuid " + ec.expedientID + ": " + e.getMessage(),
                    e);
        }
    ...
    @Override
    public void saveAll(String custodyID, Map<String, Object> custodyParameters, DocumentCustody documentCustody,
            SignatureCustody signatureCustody, Metadata[] metadata)
        ...
        try {
            ...
            if (doc == null) {
                throw new CustodyException("El document amb uuid " + ecd.documentID
                        + " no existeix en l'expedient/carpeta (" + ecd.expedientID + "/" + ecd.carpetaID + ")");
            }
            ...
            if (csv2 == null) {
                log.error(
                        "S'ha intentat llegir el CSV del fitxer amb uuid " + ecd.documentID + " però aquest val null");
            }
            ...
            */
            ...
            final Map<String, Object> generatedMetadataCollection = getMetadadesPerDocument(custodyParameters,
                    signatureCustody);
            ...
            if (documentElectronicID == null) {
                ...
                String metasAndInfoStr = (String) metadataCollection.get(MetadataConstants.ENI_DESCRIPCION);
                ...
                if (metasAndInfo.containsKey(MetadataConstants.ENI_DESCRIPCION)
                    ...
                    metadataCollection.put(MetadataConstants.ENI_DESCRIPCION,
                            metasAndInfo.getProperty(MetadataConstants.ENI_DESCRIPCION));
                }
            ...
            if (documentCustody == null) {
                ...
                metasAndInfo.setProperty(FILE_INFO_BASE_DOCUMENT + FILE_INFO_NOM, documentCustody.getName());
                metasAndInfo.setProperty(FILE_INFO_BASE_DOCUMENT + FILE_INFO_TAMANY, String.valueOf(data.length));
                if (mimetype == null) {
            ...
            if (signatureCustody == null) {
                ...
                metasAndInfo.setProperty(FILE_INFO_BASE_FIRMA + FILE_INFO_NOM, signatureCustody.getName());
                metasAndInfo.setProperty(FILE_INFO_BASE_FIRMA + FILE_INFO_TAMANY, String.valueOf(data.length));
                if (mimeType == null) {
                ...
                if (signatureCustody.getAttachedDocument() == null) {
                    ...
                    metasAndInfo.setProperty(FILE_INFO_BASE_FIRMA + FILE_INFO_SIGNATURE_ATTACHED_DOCUMENT,
            ...
            metadataCollection.put(MetadataConstants.ENI_DESCRIPCION, propertiesCustodyToStringEniDescripcion(
                    metasAndInfo, (String) metadataCollection.get(MetadataConstants.ENI_DESCRIPCION)));
            ...
            if (documentElectronicID == null) {
                ...
                if (isPropertyCreateDraft_EL(custodyParameters)) {
                    ...
                    if (hiHaError(errorCodi)) {
                        ...
                        throw new CustodyException("Error creant Document Draft amb id " + custodyID + "(" + errorCodi
                                + " - " + msg + ")");
                    }
                    ...
                    do {
                        ...
                        if (hiHaError(errorCodi)) {
                            ...
                            if ("COD_020".equals(errorCodi) && msg.startsWith("Send timeout")) {
                                log.warn("Gestió de reintents de apiArxiu.finalizarDocumento():"
                                        + " reintent compte enrera " + reintents + ". Esperam " + SLEEP_SEND_TIMEOUT
                                        + " ms");
                                Thread.sleep(SLEEP_SEND_TIMEOUT);
                            } else if ("COD_021".equals(errorCodi) && msg.startsWith(
                                    "Can not add the draft aspect to the node because is a final document")) {
                                // COD_021 - Can not add the draft aspect to the node because is a final document)
                                log.info(
                                        "S'ha intentat finalitzarDocument però aquest ja és definitiu. Es dóna per bó.");
                                break;
                            } else {
                                throw new CustodyException("Error cridant a finalizarDocumento() a "
                                        + "l'hora de crearDocumentFinal amb id " + custodyID + "(" + errorCodi + " - "
                                        + msg + ")");
                            }
                        } else {
                            break;
                        ...
                        if (reintents <= 0) {
                            throw new CustodyException("S'han esgotat els reintents i no s'ha pogut "
                                    + "tancar (finalitzar) el document amb uuid " + doc.getId() + "(" + doc.getName()
                                    + "): " + rd.getCodigoResultado() + " - " + rd.getMsjResultado());
                        }
                    } while (reintents > 0);
                ...
                if (hiHaError(codi)) {
                    ...
                    throw new CustodyException("Error actualitzant Document amb id " + custodyID + "[uuid:"
                            + documentElectronicID + "]" + "(" + codi + " - " + msg + ")");
                }
            ...
            if (isPropertyTancarExpedient_EL(custodyParameters)) {
                ...
                do {
                    ...
                    if (hiHaError(errorCodi)) {
                        ...
                        // Reintentam si COD_020-Send timeout
                        if ("COD_020".equals(errorCodi) && msg.startsWith("Send timeout")) {
                            log.warn("Gestió de reintents de apiArxiu.cerrarExpediente():" + " reintent compte enrera "
                                    + reintents + ". Esperam " + SLEEP_SEND_TIMEOUT + " ms");
                            Thread.sleep(SLEEP_SEND_TIMEOUT);
                        } else if ("COD_021".equals(errorCodi)
                    ...
                    if (reintents <= 0) {
                        throw new CustodyException(
                                "S'han esgotat els reintents i no s'ha pogut tancar l'expedient amb uuid "
                                        + ecd.expedientID + ": " + res.getCodigoResultado() + " - "
                                        + res.getMsjResultado());
                    }
                } while (reintents > 0);
            ...
            throw new CustodyException(
                    "Error creant o actualitzant document electronic " + custodyID + ": " + e.getMessage(), e);
        }
    }

    protected void checkErrorsConeguts(String code, String msg, ExpedientCarpetaDocument ecd) throws CustodyException {
        ...
        if ("COD_021".equals(code) && msg.startsWith("Duplicate child name not allowed:")) {
            ...
            throw new CustodyException("Esta intentant guardar el fitxer amb nom " + filename
                    + ", però ja n'existeix un dins aquest expedient/carpeta(" + ecd.expedientID + "/" + ecd.carpetaID
                    + ")");
        }
    }
    ...
    */
    ...
    @Override
    @Deprecated
    public void saveDocument(String custodyID, Map<String, Object> custodyParameters, DocumentCustody documentCustody)
    ...
    protected DocumentCustody internalGetDocument(String custodyID, boolean retrieveContent) throws CustodyException {
        ...
        try {
            ...
            throw new CustodyException(
                    "Error intentant obtenir contingut del Document amb custodyID " + custodyID + ": " + e.getMessage(),
                    e);
        }
    ...
    public SignatureCustody internalGetSignature(String custodyID, boolean retrieveContent) throws CustodyException {
        try {
            ApiArchivoDigital apiArxiu = getApiArxiu(null);
            ...
            String attached = p.getProperty(FILE_INFO_BASE_FIRMA + FILE_INFO_SIGNATURE_ATTACHED_DOCUMENT);
            if (attached == null) {
    ...
    @Override
    public void addMetadata(String custodyID, final Metadata metadata, Map<String, Object> custodyParameters)
            throws CustodyException, NotSupportedCustodyException, MetadataFormatException {
    ...
    @Override
    public void addMetadata(String custodyID, final Metadata[] metadataList, Map<String, Object> custodyParameters)
            throws CustodyException, NotSupportedCustodyException, MetadataFormatException {
        // TODO S'ha de gestionar multipes valors associats a un única clau
    ...
    @Override
    public void updateMetadata(String custodyID, final Metadata metadata, Map<String, Object> custodyParameters)
            throws CustodyException, NotSupportedCustodyException, MetadataFormatException {
    ...
    @Override
    public void updateMetadata(String custodyID, final Metadata[] metadatas, Map<String, Object> custodyParameters)
            throws CustodyException, NotSupportedCustodyException, MetadataFormatException {
        ...
        try {
            ...
            FullInfoDocumentElectronic fullDoc = getFullInfoOfDocumentElectronic(apiArxiu, uuid, retrieveContent);
            ...
            for (Metadata metadata : metadatas) {
                ...
                if ((key.startsWith("eni:") || key.startsWith("gdib:"))) {
                    ...
                    if (MetadataConstants.ENI_DESCRIPCION.equals(key) || MetadataConstants.ENI_CSV.equals(key)) {
            ...
            if (hiHaError(errorCodi)) {
                String msg = rd.getMsjResultado();
                throw new CustodyException(
                        "Error actualitzant Metadades " + custodyID + "(" + errorCodi + " - " + msg + ")");
            }
            ...
            throw new CustodyException(
                    "Error intentant actualitzar les metadades de " + custodyID + ": " + e.getMessage(), e);
        }
    ...
    protected Map<String, List<Metadata>> internalGetAllMetadatas(String custodyID,
        ...
        try {
            ...
            */
            ...
            throw new CustodyException("Error intentant obtenir les metadades de " + custodyID + ": " + e.getMessage(),
                    e);
        }
    ...
    @Override
    public List<Metadata> getMetadata(String custodyID, String key)
            throws CustodyException, NotSupportedCustodyException {
    ...
    @Override
    public List<Metadata> deleteMetadata(String custodyID, final String key) throws CustodyException {

        return deleteMetadata(custodyID, new String[] { key });
    }

    @Override
    public List<Metadata> deleteMetadata(String custodyID, final String[] keys) throws CustodyException {

        final boolean deleteAll = false;
        return internalDeleteMetadata(custodyID, keys, deleteAll);
    }

    protected List<Metadata> internalDeleteMetadata(String custodyID, final String[] keys, boolean deleteAll)
            throws CustodyException {
    ...
    @Override
    public void deleteAnnex(String custodyID, String annexID) throws CustodyException, NotSupportedCustodyException {
        throw new NotSupportedCustodyException();

    }

    @Override
    public void deleteAllAnnexes(String custodyID) throws CustodyException, NotSupportedCustodyException {
        throw new NotSupportedCustodyException();
    ...
    @Override
    public AnnexCustody getAnnexInfoOnly(String custodyID, String annexID) throws CustodyException {
        throw new CustodyException("No es suporten Annexes");
    }
    ...
    protected String processEL(String template, Map<String, Object> custodyParameters) throws CustodyException {
        return AbstractDocumentCustodyPlugin.processExpressionLanguage(template, custodyParameters);
    }
    ...
    protected String processELNullIfEmpty(String template, Map<String, Object> custodyParameters)
        ...
        String tmp = AbstractDocumentCustodyPlugin.processExpressionLanguage(template, custodyParameters);
    ...
    public ApiArchivoDigital getApiArxiu(Map<String, Object> custodyParameters) throws CustodyException {
        ...
        try {
            ...
            if (apiArxiuCache == null) {
                ...
                // info login
                // + Ciutadà
                String ciutadaNom = processOptionalProperty("SolicitantNom", getPropertySolicitantNomEL(),
                        custodyParameters);
                if (debug) {
                ...
                String funcionariUsername = processOptionalProperty("UsuariUsername", getPropertyUsuariUsernameEL(),
                        custodyParameters);
                ...
                if (isPropertyIgnoreServerCeriticates()) {
                    throw new UnsupportedOperationException("La propietat [" + IGNORE_SERVER_CERTIFICATE
            ...
            throw new CustodyException("Error instanciant l'API d'Arxiu Electrònic: " + e.getMessage(), e);
        }
    }
    ...
    private String processOptionalProperty(String titol, String propertyValue, Map<String, Object> custodyParameters) {
        ...
        try {
            ...
            log.warn("S'ha intentat processar la propietat titulada " + titol + " amb valor " + propertyValue
                    + " però s'ha produït un error al fer el processament EL: " + e.getMessage(), e);
            return null;
        }
     ...
     * @throws CustodyException
     */
    protected List<Metadata> recollectAutomaticMetadatas(Map<String, Object> custodyParameters, String prefix)
            throws CustodyException {
        ...
        List<Metadata> list = AbstractDocumentCustodyPlugin.recollectAutomaticMetadatas(this, custodyParameters,
                getPropertyBase() + prefix + ".", ignoreErrors);

        if (list == null || list.isEmpty()) {
            if (debug) {
                log.info("recollectAutomaticMetadatas::LIST NULL or Empty: " + propertyBase + "(" + list + ")");
            }
    ...
    protected String createSimpleDocCSV(ApiArchivoDigital api, String csv, String expedientID, String carpetaID)
        ...
        if (hiHaError(codi)) {
            throw new CustodyException("Error creant DocumentSimple dins carpeta/Expedient" + " amb uuid " + uuidParent
                    + "(" + codi + " - " + result.getCreateDraftDocumentResult().getResult().getDescription() + ")");
        }
    ...
    protected String readSimpleDocCSV(ApiArchivoDigital api, String uuidDoc) throws IOException {
        ...
        if (hiHaError(resultat.getCodigoResultado())) {
            log.error("No s'ha pogut llegir fitxer de CSV amb uuid " + uuidDoc + "(" + resultat.getCodigoResultado()
                    + ":" + resultat.getMsjResultado() + ")");
            return null;
        }
        ...
        if (csv == null) {
            log.error("S'ha intentat llegir el CSV del fitxer amb uuid " + uuidDoc + " però aquest val null");
        }
    ...
    protected Map<String, Nodo> getNodosByExpedientCarpeta(ApiArchivoDigital apiArxiu,
        ...
        if (carpetaID == null) {
            ...
            if (hiHaError(expedient.getCodigoResultado())) {
                throw new CustodyException("Error intentant obtenir informació de l'expedient amb uuid " + expedientID
                        + ": " + expedient.getCodigoResultado() + "-" + expedient.getMsjResultado());
            }
            ...
            if (hiHaError(dir.getCodigoResultado())) {
                throw new CustodyException("Error intentant obtenir informació de la carpeta amb uuid " + carpetaID
    ...
    protected FullInfoDocumentElectronic getFullInfoOfDocumentElectronic(ApiArchivoDigital apiArxiu, String uuid,
            boolean retrieveContent) throws CustodyException, IOException {
    ...
    protected Documento getDocumento(ApiArchivoDigital apiArxiu, String uuid, boolean retrieveContent)
            throws IOException, CustodyException {
        ...
        if (hiHaError(doc.getCodigoResultado())) {
            if ("COD_021".equals(doc.getCodigoResultado()) && doc.getMsjResultado().startsWith("nodeId is not valid")) {
                return null;
            } else {
    ...
    protected Properties getMetadadesSenseEni(Properties metadades, Map<String, Object> metadataDocs) {
